...
import { Calendar, Clock, MapPin, Users, Plane, Star } from 'lucide-react';
...
const FlightBooking = () => {
  ...
  const fetchFlightDetails = async () => {
    try {
      setLoading(true);
      const flightData = await flightsAPI.getFlightById(flightId);
      setFlight(flightData);
      if (flightData.classes && flightData.classes.length > 0) {
        setSelectedClass(flightData.classes[0].class_type);
      }
    } catch (error) {
  ...
  const calculateTotal = () => {
    ...
    const flightClass = flight.classes.find(c => c.class_type === selectedClass);
    if (!flightClass) return 0;
    
    const subtotal = flightClass.price * quantity;
  ...
  const handleContinue = () => {
    ...
    if (step === 3) {
      ...
      const bookingData = {
        ...
        eventDate: flight.departure_date,
        eventTime: flight.departure_time,
        venue: `${flight.from_airport} (${flight.from_airport_code}) → ${flight.to_airport} (${flight.to_airport_code})`
      };
  ...
  const selectedClassData = flight.classes.find(c => c.class_type === selectedClass);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Book your flight tickets</p>
        </div>
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h1 className="text-2xl font-bold mb-2">{flight.airline} {flight.flight_number}</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">{flight.from_airport} ({flight.from_airport_code})</p>
                      <p className="text-sm text-gray-600">{formatDate(flight.departure_date)} at {flight.departure_time}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <MapPin className="w-5 h-5 text-gray-400 mt-1" />
                    <div>
                      ...
                      <p className="font-semibold">{flight.to_airport} ({flight.to_airport_code})</p>
                      <p className="text-sm text-gray-600">{formatDate(flight.arrival_date)} at {flight.arrival_time}</p>
                    </div>
                  </div>
                </div>
              </CardContent>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {flight.classes.map((flightClass) => (
                      <div
                        key={flightClass.class_type}
                        onClick={() => setSelectedClass(flightClass.class_type)}
                        className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                          selectedClass === flightClass.class_type
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <h3 className="font-semibold capitalize">{flightClass.class_type.replace(/_/g, ' ')}</h3>
                            <p className="text-sm text-gray-600">
                              {flightClass.available_seats} seats available
                            </p>
                          </div>
                          <div className="text-right">
                            <div className="text-xl font-bold text-green-600">
                              {formatCurrency(flightClass.price)}
                            </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      ...
                      <div className="flex justify-between">
                        <span>{quantity} {quantity === 1 ? 'Ticket' : 'Tickets'} - {selectedClass.replace(/_/g, ' ')}</span>
