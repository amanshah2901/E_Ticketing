...
export const createBooking = async (req, res) => {
  ...
  try {
    ...
    const bookingData = {
      booking_type,
      booking_type_model: booking_type.charAt(0).toUpperCase() + booking_type.slice(1), // 'movie' -> 'Movie'
      item_id,
    ...
    await booking.save(useTransactions ? { session } : {});
    ...
    await payment.save(useTransactions ? { session } : {});

    // ------------------------
    // WALLET DEDUCTION LOGIC
    // ------------------------
    if (payment_method === "wallet") {
      const wallet = await Wallet.findOne({ user_id: userId }).session(useTransactions ? session : null);
      ...
      if (!wallet.hasSufficientBalance(grandTotal)) {
        if (useTransactions) {
          await session.abortTransaction();
          session.endSession();
        }
      ...
      await walletTransaction.save(useTransactions ? { session } : {});
    }
